...
CREATE TABLE Contenitore(
	idContenitore 		NUMBER GENERATED BY DEFAULT AS IDENTITY,
	ssnResponsabile 	VARCHAR2(16),
	tipoContenitore 	CHAR(15) CHECK(tipo IN ('Bollitore' , 'Fermentatore')),
...
CREATE TABLE MaltoAcquistato(
	...
	CONSTRAINT MALTOACQ_ORDER_PK_FK	
		FOREIGN KEY(GTIN,codLotto) REFERENCES LottoMateriaPrima(GTIN,codLotto) on delete cascade
);
...
CREATE TABLE MostoDolce(
	...
	CONSTRAINT MD_LUP_FK
		FOREIGN KEY(gtinLuppoloUsato,codLotto) REFERENCES LuppoloAcquistato(GTIN,codLotto) on delete cascade
);
...
CREATE TABLE Ammostamento(
	...
	CONSTRAINT AMM_FK_MALT	
		FOREIGN KEY(gtinMalto,codLotto) REFERENCES MaltoAcquistato(GTIN,codLotto) on delete cascade,
...
CREATE TABLE Fermentazione(
	...
	CONSTRAINT FERM_FK_BIRRP
		FOREIGN KEY(numLottoBirraProd) REFERENCES BirraProdotta(numLotto) on delete cascade
);
